...
class Card extends Component {
    ...
    star=()=>{
        if (localStorage.getItem(this.props.id)==="favourite"){
            localStorage.removeItem(this.props.id);
        }
        else{
        this.setState((state)=>({
            favourite:!state.favourite
        }),()=>{localStorage.setItem(this.props.id,"favourite")})
    }
    }
    render() {
        return (
            ...
            <div className="cardContainer">
                <svg viewBox='0 0 16 17' width='16' height='17' xmlns='http://www.w3.org/2000/svg' fill={this.state.favourite?"#ffffff":"#000"} className="star-btn" onClick={this.star}>
                    <path
